...
import 'package:aft/ATESTS/main.dart';
import 'package:aft/ATESTS/methods/firestore_methods.dart';
import 'package:aft/ATESTS/models/notification.dart';
import 'package:aft/ATESTS/models/user.dart';
import 'package:aft/ATESTS/provider/user_provider.dart';
  ...
  Widget build(BuildContext context) {
    ...
    if (notificationList.length > 0) {
      ...
      for (var i = 0; i < notificationList.length; i++) {
        ...
        if (user?.uid == notificationList[i].uploadUserId &&
            (prefs?.getBool(ConstantUtils.messageVotes) ?? true) == true &&
      ...
      for (var i = 0; i < tempReplyList.length; i++) {
        if (tempRepliesList.isEmpty) {
          ...
          if (tempReplyList.length > i) {
            if (tempReplyList[i - 1].uploadUserId ==
                    tempReplyList[i].uploadUserId &&
                tempReplyList[i - 1].uploadId == tempReplyList[i].uploadId) {
              String comments = tempReplyList[i].commentReply;
              String commentId = tempReplyList[i].uid;

              tempRepliesList[tempRepliesList.length - 1]
                  .comments
                  ?.add(comments);
              tempRepliesList[tempRepliesList.length - 1]
                  .commentId
      ...
      for (var i = 0; i < tempPlusList.length; i++) {
        if (i == 0) {
        ...
        } else if (tempPlusList[i - 1].uploadId == tempPlusList[i].uploadId &&
            tempPlusList[i - 1].uploadUserId == tempPlusList[i].uploadUserId) {
          if (tempPlusList[i - 1].plus.length > tempPlusList[i].plus.length) {
            if (i != 0 && i < tempPlusList[i].plus.length) {
              ...
              tempPlusFinal[i - 1].plus =
                  tempPlusFinal[i - 1].plus.toSet().toList();
            }
          } else if (tempPlusList[i].plus.length > 0 &&
              tempPlusList[i - 1].plus[0] != tempPlusList[i].plus[0]) {
      ...
      for (var i = 0; i < tempMinusList.length; i++) {
        ...
        } else if (tempMinusList[i - 1].uploadId == tempMinusList[i].uploadId &&
            tempMinusList[i - 1].uploadUserId ==
                tempMinusList[i].uploadUserId) {
          if (tempMinusList[i - 1].minus.length >
              tempMinusList[i].minus.length) {
            if (i != 0 && i < tempMinusList[i].minus.length) {
              ...
              tempMinusFinal[i - 1].minus =
                  tempMinusFinal[i - 1].minus.toSet().toList();
            }
          } else if (tempMinusList[i].minus.length > 0 &&
              tempMinusList[i - 1].minus[0] != tempMinusList[i].minus[0]) {
      ...
      for (var i = 0; i < tempNeturalList.length; i++) {
        ...
        } else if (tempNeturalList[i - 1].uploadId ==
                tempNeturalList[i].uploadId &&
            tempNeturalList[i - 1].uploadUserId ==
                tempNeturalList[i].uploadUserId) {
          if (tempNeturalList[i - 1].neutral.length >
              tempNeturalList[i].neutral.length) {
            if (i != 0 && i < tempNeturalList[i].neutral.length) {
              for (int j = 0; j < tempNeturalList[i].neutral.length; j++) {
                tempNeturalFinal[i - 1]
                    .neutral
                    .add(tempNeturalList[i].neutral[j]);
              }
              tempNeturalFinal[i - 1].neutral =
                  tempNeturalFinal[i - 1].neutral.toSet().toList();
            }
          } else if (tempNeturalList[i].neutral.length > 0 &&
              tempNeturalList[i - 1].neutral[0] !=
                  tempNeturalList[i].neutral[0]) {
        ...
        body: notificationFinalList.isNotEmpty
            ? ListView.builder(
                ...
                itemBuilder: (context, index) {
                  var username = "";
                  FirestoreMethods()
                      .getUsername(notificationFinalList[index].uid)
                  ...
                  if (notificationFinalList[index].typeStatus == "plusVote" &&
                    ...
                    return commonClick(
                        ...
                        "${notificationFinalList[index].commentId?.length}",
                        " reply", () {
                      if (notificationFinalList[index].type == "post") {
                        Navigator.push(
                          context,
